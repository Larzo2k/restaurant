                ...
                <tbody class="text-gray-600">
                    @forelse ($proveedores as $proveedor)
                        <tr class="odd">
                            ...
                            <td class="col-1">
                                <img src="{{asset($proveedor->image)}}" height="40" alt="">
                            </td>
                            <td>{{ $proveedor->name }}</td>
                            <td>{{ $proveedor->address }}</td>
                            <td>{{ $proveedor->email }}</td>
                            <td>{{ $proveedor->cod }} {{ $proveedor->phone }}</td>
                            <td class="text-end">
                                {{-- @can('editar directivos') --}}
                                    <a onclick="editar(`{{$proveedor->id}}`,`{{$proveedor->name}}`, `{{$proveedor->address}}`, `{{$proveedor->email}}`, `{{$proveedor->cod}}`, `{{$proveedor->phone}}`, `{{$proveedor->image}}`)"
                                ...
                                {{-- @endcan --}}
                                  <a onclick="eliminar(`{{$proveedor->id}}`, `{{$proveedor->name}}`, `{{$proveedor->address}}`)" class="btn btn-icon btn-flex btn-active-light-primary w-30px h-30px me-3" data-bs-toggle="tooltip" title="" data-kt-action="product_remove" data-bs-original-title="Dar de baja">
